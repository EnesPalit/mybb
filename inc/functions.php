<?php
require 'inc/composer/vendor/autoload.php';
/**
 * MyBB 1.8
...
function my_mail($to, $subject, $message, $from="", $charset="", $headers="", $keep_alive=false, $format="text", $message_text="", $return_email="")
{
    ...
    if(!is_object($mail))
    {
        ...
        if($mybb->settings['mail_handler'] == 'smtp')
        {
            ...
            require_once MYBB_ROOT."inc/mailhandlers/swiftMailer.php";
    ...
    if($mybb->settings['mail_handler'] == 'smtp')
    {
        ...
        // Try the SwiftMailer Library
        try {
            // Create the SMTP Transport
            $mail->setTransport((new Swift_SmtpTransport($host, $port, $protocol))
                ->setUsername($username)
                ->setPassword($password));

            // Create the Mailer using your created Transport
            $transport = $mail->getTransport();
            $mail->setMailer(new Swift_Mailer($transport));

            // Build and send
            $mail->buildSwiftMessage($to, $subject, $message, $from, $charset, $headers, $format, $message_text, $return_email);
            ...
            {
                $functionUsed = $mail->getFunctionUsed();
                $this->fatal_error("MyBB was unable to send the email using the PHP {$functionUsed} Library.");
                return false;
            }
            // Catch the errors
        } catch (Swift_SwiftException $e) {
            $mail->setFunctionUsed('swiftMailer()');
            $functionUsed = $mail->getFunctionUsed();
            $eMessage = $e->getMessage();
            $eCode = $e->getCode();
            $mail->fatal_error("MyBB was unable to send the email using the PHP {$functionUsed} Library. Message: {$eMessage}. Code: {$eCode}");
            return false;
        }
    }
